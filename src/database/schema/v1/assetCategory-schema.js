...
const AssetCategory = db.v1.sequelize.define(tableName, model, {
   tableName,
});
...
schemaUtils.configureRelationships(AssetCategory, relationships);
const createdByUserFkn = 'createdByUserId';
User.hasMany(AssetCategory, {
   as: `${tableName}Cbu`,
   foreignKey: createdByUserFkn,
});
AssetCategory.belongsTo(User, {
   as: 'createdByUser',
   foreignKey: createdByUserFkn,
});
...
Entity.hasMany(AssetCategory, {
   as: Entity.assetCategoryChildName,
   foreignKey: entityFkn,
});
AssetCategory.belongsTo(Entity, {
   as: AssetCategory.entityParentName,
   foreignKey: entityFkn,
});
