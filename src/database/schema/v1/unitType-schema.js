...
const relationships = {
   meterChildName: 'meters',
};

// Configure the entity to export
const tableName = 'unitType';
const model = {
   id: {
      type: Sequelize.UUID,
      primaryKey: true,
...
const UnitType = db.v1.sequelize.define(tableName, model, {
   tableName,
});
...
schemaUtils.configureRelationships(UnitType, relationships);
const createdByUserFkn = 'createdByUserId';
User.hasMany(UnitType, {
   as: `${tableName}Cbu`,
   foreignKey: createdByUserFkn,
});
UnitType.belongsTo(User, {
   as: 'createdByUser',
   foreignKey: createdByUserFkn,
});
